...
export function initializeAdminDatabase(db, dbAsync) {
  db.serialize(() => {
    ...
    // MFA tokens table
    db.run(`CREATE TABLE IF NOT EXISTS mfa_tokens (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      ...
      if (err) console.error('Error creating mfa_tokens table:', err);
      else console.log('MFA tokens table ready');
    });
    ...
    db.run(`CREATE TABLE IF NOT EXISTS system_audit_logs (
      ...
      if (err) console.error('Error creating system_audit_logs table:', err);
      else console.log('System audit logs table ready');
    });
    ...
    db.run(`CREATE TABLE IF NOT EXISTS system_alerts (
      ...
      else console.log('System alerts table ready');
    });
    ...
    db.run(`CREATE TABLE IF NOT EXISTS system_config (
      ...
      else console.log('System config table ready');
    });
    ...
    db.run(`CREATE TABLE IF NOT EXISTS admin_permissions (
      ...
      else console.log('Admin permissions table ready');
    });
    ...
    db.run(`CREATE INDEX IF NOT EXISTS idx_system_audit_logs_admin ON system_audit_logs(admin_id)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_system_audit_logs_created ON system_audit_logs(created_at)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_system_alerts_severity ON system_alerts(severity)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_mfa_tokens_admin ON mfa_tokens(admin_id)`);
  });
}
...
function initializeDefaultPermissions(db) {
  ...
  for (const [role, perms] of Object.entries(permissions)) {
    perms.forEach(permission => {
      db.run(
        `INSERT OR IGNORE INTO admin_permissions (role, permission) VALUES (?, ?)`,
        [role, permission],
        (err) => {
          if (err) console.error(`Error inserting permission ${permission} for ${role}:`, err);
        }
      );
    });
  }
...
/**
 * Seed default role-based permissions using dbAsync (works for Postgres and SQLite).
 */
export async function seedDefaultPermissionsPgSafe(dbAsync) {
  const permissions = {
    super_admin: [
      'view_dashboard',
      'manage_admins',
